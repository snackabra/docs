...
import { arrayBufferToBase64, base64ToArrayBuffer, assemblePayload, extractPayload, jsonParseWrapper, SBCrypto } from 'snackabra';
...
function APIResponse(_target, propertyKey, descriptor) {
    const operation = descriptor.value;
...
class Server {
    ...
    handleApiCall(request, env, _ctx) {
    ...
    //   })
    // }
    handleLastMessageTimes(request, env) {
        return request.json().then((r) => Promise.all(r
            .map((room) => lastTimeStamp(room, env)))
            .then((results) => results));
    }
    handleStoreRequest(request, env) {
        // console.log("handleStoreRequest()")
        const { searchParams } = new URL(request.url);
        const name = searchParams.get('name');
        const type = searchParams.get('type');
        if (!type)
            return { error: "ERROR: you need type (note: old client bug)" };
    ...
    handleMigrateStorage(request, env) {
        try {
    ...
    handleFetchDataMigration(request, env) {
        try {
            const { searchParams } = new URL(request.url);
            const verification_token = searchParams.get('verification_token');
            // const storage_token = searchParams.get('storage_token');
...
__decorate([
    APIResponse
], Server.prototype, "handleLastMessageTimes", null);
__decorate([
    APIResponse
], Server.prototype, "handleStoreRequest", null);
__decorate([
    APIResponse
], Server.prototype, "handleStoreData", null);
__decorate([
    APIResponse
], Server.prototype, "handleFetchData", null);
__decorate([
    APIResponse
], Server.prototype, "handleMigrateStorage", null);
__decorate([
    APIResponse
], Server.prototype, "handleFetchDataMigration", null);
...
class ChannelConfig {
            ...
            try {
                ...
                storage.get(Object.keys(this.storageKeys)).then((values) => {
                    Object.keys(this.#storageKeys).forEach((key) => {
                        if (typeof values.get(key) !== 'undefined')
                            this.#storageKeys[key] = values.get(key);
                    });
                });
    ...
    /** @type {number}        */ get room_capacity() { return this.#storageKeys['room_capacity']; }
    /**                       */ set room_capacity(newCapacity) { this.#storageKeys['room_capacity'] = newCapacity; }
}
__decorate([
    Ready
], ChannelConfig.prototype, "room_capacity", null);
// =======================================================================================
// Channel API (synchronous, durable object)
export class ChannelServerAPI {
    storage;
    env;
    // sessions = []
    config;
    initializePromise = undefined; // requires strictNullChecks to be false in tsconfig
    // // We keep track of the last-seen message's timestamp just so that we can assign monotonically
    // // increasing timestamps even if multiple messages arrive simultaneously (see below).
    // lastTimestamp = 0;
    room_id = '';
...
__decorate([
    APIResponse
], ChannelServerAPI.prototype, "getRoomCapacity", null);
